...
public class Commands {
    ...
    public static String differentiate(String expression, String variable) {
        if (!variable.matches("[a-zA-Z]+")) {
            throw new IllegalArgumentException(String.format("Variable name %s is invalid", variable));
        }
        return parse(expression).differentiate(variable).toString();
    }
                        ...
                        (variable, value) -> {
                            if (!variable.matches(("[a-zA-Z]+"))) {
                                throw new IllegalArgumentException(String.format(
                                        "Variable name %s is invalid", variable));
                            }
                            if (value < 0) {
                                throw new IllegalArgumentException(String.format(
                                        "Value %s of variable %s is invalid", value, variable));
                            }
                        });
        return parse(expression).simplify(environment).toString();
    }
